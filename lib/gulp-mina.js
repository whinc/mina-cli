...
module.exports = function gulpMina(options = {}) {
  const extnameMap = options.extnameMap
  ...
  var stream = through.obj(function(file, enc, cb) {
    ...
    if (file.isBuffer()) {
      ...
      const templateFile = createFile(file, result.template ? result.template.content : '', {extname: extnameMap.template})
      this.push(templateFile);

      const scriptFile = createFile(file, result.script ? result.script.content : '', {extname: extnameMap.script})
      this.push(scriptFile)

      const configBlock = result.customBlocks.find(block => block.type === 'config')
      const configFile = createFile(file, configBlock ? configBlock.content : '', {extname: extnameMap.config})
